	...
	>
		...
		<defs>
			<linearGradient id={`${uid}-dpad`} x1="0" y1="0" x2="0.3" y2="1">
				<stop stop-color="#777" /><stop offset="0.45" stop-color="#555" /><stop
					offset="1"
					stop-color="#393939"
				/>
			</linearGradient>
		...
		{/if}
		<g class="controller-shell" opacity={theme === 'enlarged' ? 0.25 : 1}>
		...
		{#if large}
			{#each [{ id: 'L', x: 48 }, { id: 'R', x: 302 }] as shoulder}
				<g class="neutral-button" class:pressed={controller.buttons[shoulder.id as 'L' | 'R']}>
					<rect x={shoulder.x} y="44" width="90" height="34" rx="10" />
					<text x={shoulder.x + 45} y="61">{shoulder.id}</text>
				</g>
			{/each}
		{/if}
		<g transform={large ? 'translate(89 165) scale(1.45) translate(-89 -165)' : undefined}>
			{#if !large}<circle
					cx="89"
					cy="165"
					r="44"
		...
		{#each face as button}
			<g
				transform={large
					? `translate(${enlargedPositions[button.id][0]} ${enlargedPositions[button.id][1]}) scale(1.65) translate(${-button.x} ${-button.y})`
					: undefined}
				class="face-button"
			...
			>
				...
				/>
				{#if button.id.startsWith('C')}
					<g class="c-arrow" transform={`translate(${button.x} ${button.y})`}
						><DirectionArrow direction={button.id} /></g
					>
				{:else}
					<text x={button.x} y={button.y + 1} class:start={button.id === 'START'}
						>{button.label}</text
					>
				{/if}
			</g>
		...
		<g
			...
			transform={large ? 'translate(220 292) scale(1.2) translate(-220 -258)' : undefined}
		>
		...
		{#if !large}<path d="M251 320l34 32h19" class="rear-line" />{/if}
		<g
			transform={large ? 'translate(181 198) scale(1.65) translate(-323 -352)' : undefined}
			class="neutral-button rear"
			class:pressed={controller.buttons.Z}
		>
			<rect x="312" y="336" width="22" height="32" rx="7" />
			<text x="323" y="352">Z</text>
		</g>
		{#if !large}<text class="rear-label" x="373" y="352">REAR</text>{/if}
	</svg>
{/if}
...
<style>
	...
	.large .neutral-button text {
		font-size: 16px;
	}
	.large .neutral-button.pressed path,
	.large .neutral-button.pressed rect {
		fill: #a78bfa;
		stroke: #ddd2ff;
		filter: drop-shadow(0 0 5px #a78bfa);
	}
	.large .direction.pressed .press-surface {
		fill: #60d9ff;
		stroke: #caf3ff;
	}
	...
	.face-button.c-button.pressed .c-arrow {
		fill: #453800;
	}
	.stick-ring {
		fill: none;
