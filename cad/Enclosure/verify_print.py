"""Check the delivered Orca project against final v1.2 STLs and toolpaths."""
...
r=Path(__file__).resolve().parent;folder=args.slice_dir.resolve();p=folder/'Enclosure-full-enclosure-v1.2-Orca-complete.3mf'
...
with zipfile.ZipFile(p) as z:
 ...
 assert len(slice_info.findall('plate'))==3
 ...
 layout=json.loads((r/'orca-v1.2-layout.json').read_text());expected={}
 ...
 for obj in config.findall('object'):
  ...
  original=np.fromfile(r/'print/full-enclosure-v1.2'/name,dtype=dt,offset=84)['v'].astype(float)
  ...
  checks.append({'name':name,'triangles':len(t),'max_vertex_error_mm':vertex_error,'source_sha256':hashlib.sha256((r/'print/full-enclosure-v1.2'/name).read_bytes()).hexdigest()})
 assert not expected
...
pause_report=json.loads((r/'pause-v1.2-toolpath-validation.json').read_text())
assert pause_report['project_sha256']==hashlib.sha256(p.read_bytes()).hexdigest(), 'Run check_nut_pause.py on this slice first'
report={'revision':'v1.2','printer':settings['printer_settings_id'],'bed':settings['curr_bed_type'],'plate_counts':[2,9,1],'slices':result,'inherited_profile_warnings':warnings,'geometry':checks,'embedded_gcode_matches_verified_files':True,'pause_validation':pause_report,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}
(r/'orca-v1.2-validation.json').write_text(json.dumps(report,indent=2)+'\n')
