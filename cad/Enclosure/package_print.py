...
out=root/'print/full-enclosure-v1.0';out.mkdir(parents=True,exist_ok=True)
...
manifest={'revision':'v1.0','status':'Generated iteration; physical testing of the retained v1.0 release does not certify subsequent edits','files':[]}
for name,qty,material,invert in specs:
 ...
 with target.open('wb') as f:
  f.write(('Enclosure v1.0 | '+name+' | bed-oriented mm').encode().ljust(80,b' '));f.write(np.uint32(len(data)).tobytes());data.tofile(f)
...
instructions=(root/'README.md').read_text().split('## Design changes')[0]
...
with zipfile.ZipFile(root/"Enclosure-full-enclosure-v1.0.zip","w",zipfile.ZIP_DEFLATED) as z:
 for filename in [e["file"] for e in manifest["files"]]+["manifest.json","PRINT-AND-ASSEMBLE.md"]:
  z.write(out/filename,"Enclosure-full-enclosure-v1.0/"+filename)
