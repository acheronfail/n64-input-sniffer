"""Build a clean v1.0 multi-plate Orca project using the retained v1.0 release profiles."""
from pathlib import Path
import zipfile,json,uuid,xml.etree.ElementTree as E,numpy as np
r=Path(__file__).resolve().parent;src=r/'print/full-enclosure-v1.0'
...
settings['project_name']='Enclosure v1.0 - independent keepers - M2 nut pause'
...
with zipfile.ZipFile(r/'Enclosure-v1.0-Orca-input.3mf','w',zipfile.ZIP_DEFLATED) as z:
 for n,data in entries.items():z.writestr(n,data)
(r/'orca-v1.0-layout.json').write_text(json.dumps(manifest,indent=2)+'\n')
