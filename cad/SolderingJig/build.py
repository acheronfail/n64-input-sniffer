...
female=Part.Shape();female.read(str(r.parent/'n64-input-ControllerPortFemale.step'));female.translate(V(-110,-20,18))
# Male black projection is flat down, opposite the housing flat up.
...
# Rounded 50 x 90 mm base, 3 mm thick, with an open soldering/wiring well.
base=union([box(-21,-39,0,42,90,3),box(-25,-35,0,50,82,3)]+[Part.makeCylinder(4,3,V(x,y,0)) for x in [-21,21] for y in [-35,47]])
base=base.cut(box(-13,-16,-1,26,28,5))
# Keyed lower seats: male front face/rear shoulder and female flange stop sliding.
ms=box(-14,14,3,28,33,15).cut(clear(male)).cut(clear(tip,.35))
fs=box(-13,-33,3,26,15.1,15).cut(clear(female))
# Preserve the full rear terminal extrusion/prong width through the female seat.
fs=fs.cut(box(-6.4,-18.5,3,12.8,5,30))
base=base.fuse(ms).fuse(fs)
caps=[];screwrefs=[]
for name,shape,cy,top,n in [('MaleClamp',male,28,29,1),('FemaleClamp',female,-26,31,2)]:
...
# Optional screw-down holes; also leave broad side rails for a bench clamp.
for x in [-20,20]:
 for y in [-35,47]:base=base.cut(Part.makeCylinder(2.2,4,V(x,y,-.5)))
base=base.removeSplitter();bo=feature('JigBase',base,parts)
report={'revision':'v1','footprint_mm':[50,90],'connector_axis_height_mm':18,'female_terminal_passage_mm':12.8,'hardware':'Four 2 mm x 8 mm countersunk screws suitable for plastic; no nuts or print pauses','parts':{},'checks':{}}
...
paths=[box(-6.39,-18.49,12,12.78,4.4,13),box(-7,0,10,14,11.4,18)]
...
# Root STLs are already print-oriented: base upright, clamp flat top on bed.
for o in parts.Group:
 s=o.Shape.copy()
...
Part.export(parts.Group,str(r/'N64-Soldering-Jig-v1.step'))
d.recompute();d.saveAs(str(r/'N64-Soldering-Jig-v1.FCStd'))
