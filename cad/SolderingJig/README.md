...
[Print-ready 3MF](N64-Soldering-Jig-v1.3mf) · [FreeCAD model](N64-Soldering-Jig-v1.FCStd) · [STEP](N64-Soldering-Jig-v1.step)

![Assembled jig](assembled.png)

## Printing

Print one each of `JigBase.stl`, `MaleClamp.stl` and `FemaleClamp.stl`. The files are already bed-oriented: base upright and clamps upside down, with their flat top faces on the bed. Print at 100% scale. No supports, embedded nuts or pauses are required.
...
![Clamps lifted for loading](loading.png)
...
Run `build.py` using FreeCAD's Python runtime, then `check_fit.py` and `preview.FCMacro`. Run `create_3mf.py` with Python/NumPy, slice the input 3MF with OrcaSlicer, and run `verify_print.py` to check the sliced export. Connector STEP references remain in the parent CAD folder. This jig is separate from the v0.16 enclosure and does not change its parts.
