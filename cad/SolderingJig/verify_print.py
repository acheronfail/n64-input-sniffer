...
r=Path(__file__).resolve().parent;folder=Path('/tmp/n64-jig-slice');p=folder/'N64-Soldering-Jig-v1.3mf'
...
result=json.loads((folder/'result.json').read_text());assert result['return_code']==0 and len(result['sliced_plates'])==1
...
report={'revision':'v1','printer':settings['printer_settings_id'],'bed':settings['curr_bed_type'],'plate_counts':[3],'slices':result,'geometry':checks,'embedded_gcode_matches_verified_files':True,'pauses':0,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}
