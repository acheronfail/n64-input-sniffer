...
Use the [print-ready OrcaSlicer 3MF](../../N64InputMod-full-enclosure-v0.15-Orca-complete.3mf). It includes all 12 parts and checked G-code across three plates, using the existing Bambu Lab P1S 0.4 mm profile, 0.16 mm layers and a 0.20 mm first layer:
...
Both shells must be reprinted because the six closure fastener positions have moved. All port keepers, the PCB keeper and light window are unchanged and can be reused. Each nut now has a complete hex pocket, at least 1.5 mm of surrounding wall verified in CAD, and intact bearing and cap surfaces. The enclosure is 18 mm deeper toward the console, enclosing almost all of each male housing. Eight separate keepers attach to the lower shell, so lifting the lid leaves the ports secured. The lower shell remains the structural base; this is not a separate removable internal carrier.

Male housings are captured by their contours and a front retaining lip around the smaller mating portion. There are **no screws into connector housings, no TPU pads, no pressure shoes, and no M3 clamp hardware**. Female keepers capture the existing flange. Hardware positions remain those of v0.13; the board, board keeper, LED window and USB opening move 18 mm toward the console together.
...
1. Check that the real male mating portion agrees with the extended CAD reference: 16 mm circular diameter, flattened to 12 mm high, projecting 12 mm from the housing. The model assumes it is concentric with the housing and has its flat side facing the housing's flat side (up). The 0.35 mm mating-opening clearance is per axis. Contacts are not modelled.
