# N64InputMod v0.3 — full enclosure print set

The owner confirmed the board mount/keeper, USB opening and connector seat fits, and chose to proceed without a separate male-clamp retention test. The enclosure geometry is unchanged from v0.3. These STL copies are rotated/translated for printing; the original CAD/STLs retain assembly coordinates.
...
| BoardKeeper.stl | 0 | Reuse the tested keeper; spare file included |
...
- Duplicate the shoe and pad to four copies each. The keeper is only a spare.
- Print rigid parts, soft pads and the translucent window in separate material jobs as appropriate. No sliced printer profile or G-code is included.
- Inspect the sliced preview, particularly supports/bridges around connector openings, the USB opening and internal nut pockets. Preserve the fit surfaces when removing supports.
...
1. Seat the board in the lower shell with its USB connector through the front opening. Reuse the U-shaped keeper: open end toward USB, fingers over the PCB corners. Attach it using the two M2 screws, seating the keeper on its bosses.
