...
if uz < 0:
 base=base.cut(box(-tab_w/2,fr-8,uz,tab_w,tab_y-(fr-8),30))
 tongue=shell.common(box(-tab_w/2+gap,fr-8,uz+gap,tab_w-2*gap,tab_y-(fr-8),1-uz-gap))
 lid=lid.fuse(tongue).removeSplitter()
else:
 lid=lid.cut(box(-tab_w/2,fr-8,gap,tab_w,tab_y-(fr-8),uz))
 tongue=shell.common(box(-tab_w/2+gap,fr-8,-.2,tab_w-2*gap,tab_y-(fr-8),uz+.2))
...
wi=feature('LightWindow',box(-8.8,fr+5.2,13.95,17.6,17.6,1.7),parts,(.65,.9,.83));wi.Label='Optional translucent LED window'
