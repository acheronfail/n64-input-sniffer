...
out=root/'print/full-enclosure-v0.10';out.mkdir(parents=True,exist_ok=True)
dtype=np.dtype([('n','<f4',3),('v','<f4',(3,3)),('a','<u2')])
specs=[('LowerShell',1,'Use the material/profile that passed the coupon test',False),('UpperShell',1,'Same as lower shell',True),('BoardKeeper',1,'Rigid material matching the enclosure',False),('MaleClampShoe',4,'Rigid material matching the enclosure',False),('MaleClampPad',4,'Flexible TPU, or cut equivalent soft rubber sheet',False),('LightWindow',1,'Optional translucent material',False)]
manifest={'revision':'v0.10','status':'Proceeding to full enclosure at owner request; separate male retention gauge test waived, not physically verified','files':[]}
for name,qty,material,invert in specs:
 ...
 with target.open('wb') as f:
  f.write(('N64InputMod v0.10 | '+name+' | bed-oriented mm').encode().ljust(80,b' '));f.write(np.uint32(len(data)).tobytes());data.tofile(f)
...
with zipfile.ZipFile(root/"N64InputMod-full-enclosure-v0.10.zip","w",zipfile.ZIP_DEFLATED) as z:
 for filename in [e["file"] for e in manifest["files"]]+["manifest.json","PRINT-AND-ASSEMBLE.md"]:
  z.write(out/filename,"N64InputMod-full-enclosure-v0.10/"+filename)
