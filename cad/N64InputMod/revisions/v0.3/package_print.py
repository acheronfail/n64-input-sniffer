...
out=root/'print/full-enclosure-v0.3';out.mkdir(parents=True,exist_ok=True)
dtype=np.dtype([('n','<f4',3),('v','<f4',(3,3)),('a','<u2')])
specs=[('LowerShell',1,'Use the material/profile that passed the coupon test',False),('UpperShell',1,'Same as lower shell',True),('BoardKeeper',0,'Reuse the tested keeper; file supplied as a spare',False),('MaleClampShoe',4,'Rigid material matching the enclosure',False),('MaleClampPad',4,'Flexible TPU, or cut equivalent soft rubber sheet',False),('LightWindow',1,'Optional translucent material',False)]
manifest={'revision':'v0.3','status':'Proceeding to full enclosure at owner request; separate male retention gauge test waived, not physically verified','files':[]}
for name,qty,material,invert in specs:
 ...
 with target.open('wb') as f:
  f.write(('N64InputMod v0.3 | '+name+' | bed-oriented mm').encode().ljust(80,b' '));f.write(np.uint32(len(data)).tobytes());data.tofile(f)
