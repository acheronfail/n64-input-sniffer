...
# The new metal-plug path passes from socket face to outside the rear wall.
...
previous=A.openDocument(str(r/'revisions/v0.11/N64InputMod.FCStd'))
for name in ['BoardKeeper','ESP32BoardEnvelope','USBCEnvelope']:
 checks[name+'_unchanged_from_v011_mm3']=change(previous.getObject(name).Shape,new.getObject(name).Shape)
for name in ['LowerShell','UpperShell']:
 checks[name+'_added_material_vs_v011_mm3']=vol(new.getObject(name).Shape.cut(previous.getObject(name).Shape))
 checks[name+'_shroud_remaining_mm3']=vol(new.getObject(name).Shape.common(box(-12,-8,4,24,10,14)))
assert new.getObject('USBAccessGuard') is None
...
result={'revision':'v0.12','checks':checks,'dimensions':{'PCB_bounds':str(new.ESP32BoardEnvelope.Shape.BoundBox),'USB_socket_face_y_mm':face,'keeper_to_upper_shell_clearance_mm':new.BoardKeeper.Shape.distToShape(new.UpperShell.Shape)[0],'keeper_head_to_lid_clearance_mm':min(h.distToShape(new.UpperShell.Shape)[0] for h in heads)},'limits':['Console scan alignment is approximate.','Rear-facing USB and the warnings are the intended precaution; this design is not an electrical or mechanical interlock.','Disconnect all four console plugs before connecting USB-C.']}
