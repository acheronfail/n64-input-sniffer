...
out=root/'print/full-enclosure-v0.12';out.mkdir(parents=True,exist_ok=True)
...
manifest={'revision':'v0.12','status':'Console-facing USB access prototype; geometry and Orca slices checked; physical fit unverified','files':[]}
for name,qty,material,invert in specs:
 ...
 with target.open('wb') as f:
  f.write(('N64InputMod v0.12 | '+name+' | bed-oriented mm').encode().ljust(80,b' '));f.write(np.uint32(len(data)).tobytes());data.tofile(f)
...
with zipfile.ZipFile(root/"N64InputMod-full-enclosure-v0.12.zip","w",zipfile.ZIP_DEFLATED) as z:
 for filename in [e["file"] for e in manifest["files"]]+["manifest.json","PRINT-AND-ASSEMBLE.md"]:
  z.write(out/filename,"N64InputMod-full-enclosure-v0.12/"+filename)
