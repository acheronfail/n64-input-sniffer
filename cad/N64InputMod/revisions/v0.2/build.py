...
P={'port_x':[-63.7,-36,36,63.7],'rear_y':-9.0,'front_y':-62.0,'half_width':84.0,'bottom_z':-16.0,'top_z':16.0,'wall':2.4,'connector_clearance':0.22,'nose_y':7.0,'curve_radius':1450.0,'pcb_width':18.0,'pcb_length':22.52,'pcb_thickness':1.6,'pcb_z':-8.6,'pcb_front_y':-59.0,'pcb_clearance':0.2,'keeper_gap':0.15,'keeper_thickness':1.8,'seam_gap':0.16}
...
# Six M2.5 self-tapping lid fasteners, accessible underneath.
...
# Move USB access with the lower PCB. Cable overmould clearance remains 15.4 x 4.4.
usb=union([box(-5.5,-68,pcbz+1,11,12,4.4),Part.makeCylinder(2.2,12,V(-5.5,-68,pcbz+3.2),V(0,1,0)),Part.makeCylinder(2.2,12,V(5.5,-68,pcbz+3.2),V(0,1,0))])
shell=shell.cut(usb)
# Independent male-housing pressure shoes. Captive M3 nuts load the roof upwards;
# nuts enter from underneath before the upper half is installed.
...
for i,x in enumerate(P['port_x']):
 ...
 guide=box(x-6.2,cy_m-4.2,6.5,12.4,8.4,4.3)
 ...
 screw=Part.makeCylinder(1.65,8,V(x,cy_m,10))
 for tool in [guide,nut,screw]:shell=shell.cut(tool)
...
# Short, rigid cradle. No tall flexible columns: positive retention uses a removable keeper.
...
keeper=union(keep)
for x in [-13,13]:keeper=keeper.cut(Part.makeCylinder(1.1,kh+1,V(x,cy,keeperz-.5)))
ko=feature('BoardKeeper',keeper.removeSplitter(),parts,(.32,.37,.40));ko.Label='PCB keeper · two M2 x 8 screws · print flat'
...
bo=feature('LowerShell',base,parts);bo.Label='Lower shell · rigid PCB cradle · M2 keeper'
...
bc=feature('BoardFitCoupon',base.common(box(-18,usby-2,-17,36,L+5,26)),hist)
