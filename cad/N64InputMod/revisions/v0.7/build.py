...
# Six M2 through screws with side-loaded captive nuts, accessible underneath.
...
# M2 closure clearance holes and flat-head seats; side-loaded M2 nuts in lid.
for x,y in screws:
 base=base.cut(Part.makeCylinder(1.1,19,V(x,y,-17)))
 base=base.cut(countersink(x,y,bz,P['closure_head_diameter'],2.2,1))
 # Nut pockets sit above a 1.84 mm floor. Load from the cavity before closure.
 ...
 if along_y:slot=box(x-af/2,y-11,nz,af,11,nh)
 elif x>0:slot=box(x-6,y-af/2,nz,6,af,nh)
 else:slot=box(x,y-af/2,nz,6,af,nh)
 lid=lid.cut(pocket.fuse(slot))
