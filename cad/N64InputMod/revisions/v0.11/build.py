...
for i,x in enumerate(P['port_x']):
 ...
 collars.extend([box(x-13.6,-28+bow(x),-13.6,27.2,21.5,27.2),box(x-13.2,fr,bz+wall,26.4,19,13.6-(bz+wall))])
 # Cable/solder access from housing tails to common cavity.
 cutters.extend([box(x-7,y-38,-5,14,5,10),box(x-9,fr+16,-5,18,10,10)])
...
shell=shell.cut(usb)
if P.get('pcb_console_side',0):
 # Console-facing access tunnel: the socket-sized opening remains in the case
 # wall; this wider tunnel admits a cable moulding only from the console side.
 # Its end is close to the approximately registered scan, blocking side access.
 gw=P['usb_guard_wall'];giw=P['usb_guard_inner_width'];gih=P['usb_guard_inner_height']
 gz=P['usb_guard_inner_bottom_z'];gy=re-2;ge=P['usb_guard_end_y']
 guard_outer=box(-giw/2-gw,gy,gz-gw,giw+2*gw,ge-gy,gih+2*gw)
 guard_bore=box(-giw/2,gy-.1,gz,giw,ge-gy+.2,gih)
 guard=guard_outer.cut(guard_bore)
 shell=shell.fuse(guard).removeSplitter()
 feature('USBAccessGuard',guard)
...
tab_start=fr-(12 if P.get('pcb_console_side',0) else 8)
if uz < 0:
...
bo=feature('LowerShell',base,parts);bo.Label='Lower shell · console-facing ESP pedestal · rear split USB opening'
lo=feature('UpperShell',lid,parts);lo.Label='Upper shell · rear USB notch · relocated LED window · male clamps'
