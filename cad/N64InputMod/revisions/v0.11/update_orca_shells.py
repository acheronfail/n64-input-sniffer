...
for name in ['LowerShell','UpperShell','BoardKeeper']:
 ...
 raw=(r/'print/full-enclosure-v0.11'/(name+'.stl')).read_bytes();vs=[];index={};ts=[]
 ...
 if name=='LowerShell':
  for key,val in [('enable_support','1'),('support_on_build_plate_only','1')]:
   md=obj.find(f"metadata[@key='{key}']")
   if md is None:md=E.SubElement(obj,'metadata',key=key)
   md.set('value',val)
 part=obj.find('part')
...
for key in list(data):
 if key.endswith('.png') or key in ['Metadata/plate_1.json','Metadata/slice_info.config']:
  del data[key]
s=json.loads(data['Metadata/project_settings.config']);s['project_name']='N64InputMod v0.11 - console-facing USB with nut pauses';data['Metadata/project_settings.config']=json.dumps(s,indent=2).encode()
with zipfile.ZipFile(r/'N64InputMod-full-enclosure-v0.11-Orca-complete.3mf','w',zipfile.ZIP_DEFLATED) as z:
