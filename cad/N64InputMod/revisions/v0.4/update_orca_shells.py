...
for name in ['LowerShell','UpperShell']:
 ...
 raw=(r/'print/full-enclosure-v0.4'/(name+'.stl')).read_bytes();vs=[];index={};ts=[]
...
s=json.loads(data['Metadata/project_settings.config']);s['project_name']='N64InputMod v0.4 complete - split USB seam';data['Metadata/project_settings.config']=json.dumps(s,indent=2).encode()
with zipfile.ZipFile(r/'N64InputMod-full-enclosure-v0.4-Orca-complete.3mf','w',zipfile.ZIP_DEFLATED) as z:
