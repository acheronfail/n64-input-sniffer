# N64InputMod — scan-measured male spacing v0.13

Open [N64InputMod.FCStd](N64InputMod.FCStd), also saved as `N64InputMod-v0.13.FCStd`. The v0.12 CAD, source and checks are preserved in `revisions/v0.12/`; its print set and Orca project remain historical.

## Current revision: outer male connector spacing

The v0.12 physical print was difficult to connect because the male connectors sat slightly too far inward. Exact sections of both original 120 MB scan meshes measure the outer half-spacing as **64.042 mm** and the inner half-spacing as **36.013 mm**, after removing the scans' approximately +0.5 mm lateral origin offset. v0.13 sets male X centres to **−64.04, −36.00, +36.00, +64.04 mm**: the outer two move **0.34 mm outward each** and their span increases **0.68 mm**. The inner pair stays fixed.

`male_port_x` controls the male seats, collars, tail relief, clamp guides, nuts, screws, pads and shoes together. `port_x` retains the female positions and the previous male Y-depth reference. Male Y and Z, female seats, enclosure outline, board and USB geometry are unchanged. The shell opening scan supports lateral spacing; it does not certify installed socket/contact positions or printer dimensional accuracy.

Reprint both shells using [the v0.13 STL package](N64InputMod-full-enclosure-v0.13.zip) and [assembly notes](print/full-enclosure-v0.13/PRINT-AND-ASSEMBLE.md). Reuse the shoes, pads, keeper and window. Re-slice the new shells; the v0.12 3MF has old spacing and must not be printed unchanged. Standalone STLs do not contain nut-insertion pauses.

Measurement evidence: [comparison plot](port-spacing-comparison.png), `port-spacing-measurements.json`, and reproducible `measure_port_spacing.py`. Validation: `port-spacing-validation.json` checks exact male translations, unchanged female/board geometry, local seat translations, changes confined to the outer mounts, lid assembly sweep and USB access; `validation.json` checks valid solids and hardware collisions; `mesh-validation.json` checks STL closure. Physical fit of v0.13 remains unverified.

## Retained electrical precautions
...
## Earlier v0.12 revision: USB access from the console side
...
The USB shroud has been removed at the owner’s request to simplify printing. The console-facing board position, keeper, LED window, 1.5 mm additional mount inset and 10 × 4 mm split USB opening are unchanged. The socket face remains at Y = −11.8 mm, about 2.8 mm behind the central rear case face. The front USB opening remains closed.

The rear-facing USB location and the warnings above are the intended precautions. This enclosure is not an electrical or mechanical interlock: fully disconnect all four console plugs before connecting USB-C, even if the console is switched off.

## Historical v0.12 files and checks

- Native CAD: `N64InputMod.FCStd` and `N64InputMod-v0.12.FCStd`.
- STEP: `N64InputMod.step`.
- Print files: [v0.12 STLs and assembly notes](print/full-enclosure-v0.12/PRINT-AND-ASSEMBLE.md). Both shells must be reprinted. The keeper, window, shoes and pads retain their shapes and can be reused.
...
Use [the complete v0.12 OrcaSlicer 3MF](N64InputMod-full-enclosure-v0.12-Orca-complete.3mf). It contains all 12 parts across three plates, the existing Bambu Lab P1S 0.4 mm profile, and checked toolpaths. All three plates sliced successfully in OrcaSlicer 2.4.2 with no reported plate warnings. The shroud-specific lower-shell supports are removed; support generation is disabled in the supplied profile. Physical printing remains unverified.
...
The sections below record previous fit tests and revisions. Front-facing board instructions and old print sets describe those earlier versions, not v0.12.
...
Shell dimensions remain approximately 169 × 57.5 × 32 mm, excluding exposed connectors and clamp screw heads. The historical v0.12 port axes were X = −63.7, −36, +36, +63.7 mm; male nose Y = 7 + X²/1450 mm. v0.13 moves only the outer male X axes to ±64.04 mm while retaining those Y values. Those values still require checking on the actual console. The modelled female fronts look solid because the supplied STEP files are housing envelopes without contact openings. They are not printed connector caps.
