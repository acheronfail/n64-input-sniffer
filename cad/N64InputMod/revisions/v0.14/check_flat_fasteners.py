...
for i,(x,y) in enumerate(coords):
 ...
 checks[f'closure_nut_{i}_insertion_collision']=max(vol(nut.translated(V(0,-t,0) if abs(x)==79 else V(-t if x>0 else t,0,0)).common(d.UpperShell.Shape)) for t in [0,.5,1,2,3,4,5,6,8,10,12])
...
(r/'flat-fastener-validation.json').write_text(json.dumps(checks,indent=2)+'\n')
