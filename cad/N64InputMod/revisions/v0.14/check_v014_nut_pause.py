...
import json,re,math,zipfile,numpy as np
r=Path(__file__).resolve().parent;folder=Path('/tmp/n64-v014-slice')
project=folder/'N64InputMod-full-enclosure-v0.14-Orca-complete.3mf'
...
centres={f'M2_{i+1}':(128+x-ox,yoff-y) for i,(x,y) in enumerate([(-77,-49),(77,-49),(-19,-19),(19,-19),(-79,-18),(79,-18)])}
...
result={'revision':'v0.14','pauses':pauses,'all_six_caps_begin_after_pause':True,'other_plates_have_no_pause':True,'pockets':{name:{'centre_xy_mm':centres[name],'first_cap_extrusion':events[0],'cap_segments':len(events)} for name,events in hits.items()}}
(r/'pause-v0.14-toolpath-validation.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
