"""Build a clean v0.14 multi-plate Orca project using the saved v0.12 profiles."""
from pathlib import Path
import zipfile,json,uuid,xml.etree.ElementTree as E,numpy as np
r=Path(__file__).resolve().parent;src=r/'print/full-enclosure-v0.14'
...
settings['project_name']='N64InputMod v0.14 - independent keepers - M2 nut pause'
...
with zipfile.ZipFile(r/'N64InputMod-v0.14-Orca-input.3mf','w',zipfile.ZIP_DEFLATED) as z:
 for n,data in entries.items():z.writestr(n,data)
(r/'orca-v0.14-layout.json').write_text(json.dumps(manifest,indent=2)+'\n')
