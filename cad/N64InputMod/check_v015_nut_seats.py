...
d=A.openDocument(str(r/'N64InputMod.FCStd'));old=A.openDocument(str(r/'revisions/v0.14/N64InputMod.FCStd'));V=A.Vector
checks={};nuts=[]
...
for obj in old.PrintParts.Group:
 if obj.Name in ['LowerShell','UpperShell']:continue
...
report={'revision':'v0.15','nuts':nuts,'unchanged_parts':checks,'nut_proxy':{'across_flats_mm':4,'height_mm':1.6},'all_six_have_complete_walls_seats_and_caps':True}
(r/'nut-seats-v0.15-validation.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
