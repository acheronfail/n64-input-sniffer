"""Validate the delivered Orca project against final v0.15 STLs and toolpaths."""
from pathlib import Path
import json,zipfile,hashlib,xml.etree.ElementTree as E,numpy as np,shutil
r=Path(__file__).resolve().parent;folder=Path('/tmp/n64-v015-slice');p=folder/'N64InputMod-full-enclosure-v0.15-Orca-complete.3mf'
...
with zipfile.ZipFile(p) as z:
 ...
 layout=json.loads((r/'orca-v0.15-layout.json').read_text());expected={}
 ...
 for obj in config.findall('object'):
  ...
  original=np.fromfile(r/'print/full-enclosure-v0.15'/name,dtype=dt,offset=84)['v'].astype(float)
  ...
  checks.append({'name':name,'triangles':len(t),'max_vertex_error_mm':vertex_error,'source_sha256':hashlib.sha256((r/'print/full-enclosure-v0.15'/name).read_bytes()).hexdigest()})
...
report={'revision':'v0.15','printer':settings['printer_settings_id'],'bed':settings['curr_bed_type'],'plate_counts':[2,9,1],'slices':result,'geometry':checks,'embedded_gcode_matches_verified_files':True,'pause_validation':json.loads((r/'pause-v0.15-toolpath-validation.json').read_text()),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}
(r/'orca-v0.15-validation.json').write_text(json.dumps(report,indent=2)+'\n')
