"""Geometric regression for the independent v0.16 connector retention."""
from pathlib import Path
import sys,json,itertools
sys.path.insert(0,'/app/freecad/lib')
import FreeCAD as A,Part
r=Path(__file__).resolve().parent
old=A.openDocument(str(r/'revisions/v0.15/N64InputMod.FCStd'))
...
dy=p['rear_y']-json.loads((r/'revisions/v0.15/parameters.json').read_text())['rear_y']
for name in ['BoardKeeper','LightWindow','ESP32BoardEnvelope','ESP32ComponentKeepout','USBCEnvelope','BoardCradle','USBSeamTongue']:
 checks[name+'_translated_only_mm3']=difference(old.getObject(name).Shape.translated(v(0,dy-(p['led_window_inward_shift'] if name=='LightWindow' else 0),0)),new.getObject(name).Shape)
for i in range(1,5):
...
result={'revision':'v0.16','checks':checks,'retention_checks':retention,'physical_fit_verified':False,'console_scan_axial_clearance':'See console-retention-fit.json: extended mating envelopes checked separately under the explicitly revised scan registration; installed socket depth remains unverified.'}
