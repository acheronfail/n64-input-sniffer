...
r=Path(__file__).resolve().parent;folder=Path('/tmp/n64-v015-slice')
project=folder/'N64InputMod-full-enclosure-v0.15-Orca-complete.3mf'
...
result={'revision':'v0.15','pauses':pauses,'all_six_caps_begin_after_pause':True,'other_plates_have_no_pause':True,'pockets':{name:{'centre_xy_mm':centres[name],'first_cap_extrusion':events[0],'cap_segments':len(events)} for name,events in hits.items()}}
(r/'pause-v0.15-toolpath-validation.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
